...
A plant proves a construct's known answer once; a **journey** is a user flow that uses it. The
review behind tflw's `M239` arc found nine browser statements used exactly once outside the plants,
so a construct could be graded and still never be *used* the way a project uses it. Since
`S-3b` (decision 16) every browser statement is used by at least **three** journeys, and since
`S-3c` (decision 17) every workload shape by at least one, under `tests/` — `.env-specific/`
included, the plants, the check-only fixtures, scratch and the page's own suite not. A call to an
action counts what the action writes. The floors are `scripts/lib/journeys.mjs`'s and only ever
rise; `scripts/verify-journeys.mjs` holds the table below to the tree (`--write` regenerates it).

<!-- journeys:start — generated by `node scripts/verify-journeys.mjs --write`; never hand-edit -->
...
| `step:press` | browser | 3 | 3 |
| `step:hover` | browser | 3 | 3 |
| `step:scroll` | browser | 3 | 3 |
| `step:within` | browser | 25 | 3 |
| `step:accept` | browser | 3 | 3 |
| `step:dismiss` | browser | 3 | 3 |
| `step:switch` | browser | 3 | 3 |
| `step:close` | browser | 3 | 3 |
| `step:download` | browser | 3 | 3 |
| `step:drag` | browser | 3 | 3 |
...
| `C60` | `equals` (`matcher:equals`) | api | `body.price` is `42` and `body.truthy` is `4`, so `not equals 4` is red for an `equals` written as a prefix comparison — the implementation that passes all 1581 other uses here; plus case (`"Known-Answer"`) and whitespace (`"known-answer "`). Since `S-3d`: a failing `equals` against the edge module's 2 MiB string prints a message that says it is capped (`truncated, showing … of … chars`) and is under 20 000 chars (`tests/.constructs/edge-output.tflw`) | an `equals` that folds case, trims, or compares prefixes |
...
| `C118` | `baseline` (`config:key:baseline`) | api | tflw's baseline key, graded for the first time by a run whose verdict it moves (`M212`, tflw `M234-04`). `/leak/a` and `/leak/b` on `arrival-server.mjs` disclose a stack frame; the accepted set is minted by a narrowed `--baseline-write`, never typed. No key: **`FAIL 0/2`**. With it: `a` passes with its finding still reported as `withheld: 'baseline'`, `b` fails naming `/leak/b` only — **`FAIL 1/2`**, nothing stale. Both weaknesses fixed at the same address: **`PASS 2/2`**, and `results.json`'s `baseline.stale` holds exactly `a`'s fingerprint, also named on the console — a stale entry never gates | a `baseline` key parsed and never reaching the gate; suppression by rule instead of by fingerprint; a stale entry that fails the build or is never named |
| `C119` | `helpers` (`config:directive:helpers`) | check | the same two files under two configs (tflw `M239` `D`, `D1319`): with no `helpers` line, `use "./helpers/stamp.ts"` is clean and `use "./lib/sign.ts"` is `TF083` naming `lib/sign.ts` and the `helpers "./lib"` repair; with `helpers "./lib"` the verdicts swap over unchanged files, so a declaration replaces the defaults rather than widening them. `run --no-helpers` refuses the clean file too, naming the flag, before any request; and `check` prints one `helper …` line per module it would load | a fence that widens instead of replacing, a `use` judged from where tflw was invoked rather than from where the file sits, `--no-helpers` honoured only after the first request, and a `check` that loads code without saying so |
| `C120` | `runs keep` (`config:directive:runs`) | check | one plain file under three configs (tflw `M241` `E`, `D1325`): `runs keep 3` checks clean; `runs keep 0` is an error at `tflw.config:4` naming `runs keep`; clean again with no `runs` line. What the page does with the count is tflw's `ui-server-boundary` gate. | a directive dropped silently, a zero accepted, a refusal that names nothing |
| `C121` | `has count at least` (`matcher:has-count-at-least`) | api | tflw `M242` `A` (`D1326`): on the two-element `tags`, `at least 2` and `at least 1` hold and `not at least 3` holds — graded with `C60`-`C66` and their mutation control | a bound that is exclusive, exact, or pointing the wrong way |
| `C122` | `has count at most` (`matcher:has-count-at-most`) | api | `at most 2` and `at most 5` hold and `not at most 1` holds — `C121` mirrored | a bound that is exclusive, exact, or pointing the wrong way |
| `C123` | `is empty` (`matcher:is-empty`) | api | a non-empty list and string are `not empty`; a product search nothing matches is `empty` | an emptiness test that answers one way for every subject |
