 ...
 */
const ANCHORS = [
  /^#\s+.*?`?(D\d{1,3}[a-z]?|M\d{1,3}[a-z]?\d?)(?!-\d)(?!\.\d)`?\s*[—:-]/,
  /^#{1,5}\s+(?:\d+\.\s*)?`?(D\d{1,3}[a-z]?|M\d{1,3}[a-z]?\d?)(?!-\d)(?!\.\d)`?\s*[—.:-]/,
  /^#{1,5}\s+.*?[(`]`?(D\d{1,3}[a-z]?|M\d{1,3}[a-z]?\d?)(?!-\d)(?!\.\d)`?[),`]/,
  /^\*\*`?(D\d{1,3}[a-z]?|M\d{1,3}[a-z]?\d?)(?!-\d)(?!\.\d)`?\s*[—.:-]/,
  /^\s*[-*]\s+\*\*`?(D\d{1,3}[a-z]?|M\d{1,3}[a-z]?\d?)(?!-\d)(?!\.\d)`?\*{0,2}\s*[—.:-]/,
  /^\|\s*\*{0,2}`?(D\d{1,3}[a-z]?|M\d{1,3}[a-z]?\d?)(?!-\d)(?!\.\d)`?\*{0,2}\s*[—:-]\s/,
  /^\|\s*\*{0,2}`?(D\d{1,3}[a-z]?|M\d{1,3}[a-z]?\d?)(?!-\d)(?!\.\d)`?\*{0,2}\s*\|/,
];
