...
export const PLANTS = [
  ...
  },
  {
    ...
    evidence: { file: 'tests/.constructs/config-keys/web-admin.config', pattern: '^\\s*web (env TFLW_WEB_ADMIN_BASE default )?"http://localhost:8091"\\s*$', min: 1 },
    run: 'open-admin.tflw',
    graders: ['acceptance', 'coverage'],
    knownAnswer:
  ...
  // tflw `M242` `A` (`D1326`) — the count's two bounds and the emptiness test, graded with `C60`-`C66`
  // in one plant, one run and one mutation control.
  {
    id: 'C121',
    construct: 'matcher:has-count-at-least',
    family: 'matcher',
    tier: 'api',
    title: '`has count at least` is an inclusive lower bound',
    target: 'tests/.constructs/matcher-discrimination.tflw against `GET /v1/soft-check/known-answer` — `body.tags` has two elements',
    evidence: { file: 'tests/.constructs/matcher-discrimination.tflw', pattern: '^\\s*expect body\\.\\S+ not has count at least ', min: 1 },
    graders: ['acceptance'],
    knownAnswer: '`at least 2` and `at least 1` hold and `not at least 3` holds: an exact count fails `at least 1`, an upper bound fails it too, and a matcher stuck at true fails the negative.',
    catches: 'a bound that is exclusive, exact, or pointing the wrong way.',
    blockedOn: null,
  },
  {
    id: 'C122',
    construct: 'matcher:has-count-at-most',
    family: 'matcher',
    tier: 'api',
    title: '`has count at most` is an inclusive upper bound',
    target: 'the same plant and payload as `C121`',
    evidence: { file: 'tests/.constructs/matcher-discrimination.tflw', pattern: '^\\s*expect body\\.\\S+ not has count at most ', min: 1 },
    graders: ['acceptance'],
    knownAnswer: '`at most 2` and `at most 5` hold and `not at most 1` holds — `C121` mirrored.',
    catches: 'a bound that is exclusive, exact, or pointing the wrong way.',
    blockedOn: null,
  },
  {
    id: 'C123',
    construct: 'matcher:is-empty',
    family: 'matcher',
    tier: 'api',
    title: '`is empty` is none of a thing, and `is not empty` is some',
    target: 'the known-answer payload\'s `tags` and `label`, and a product search nothing matches (`[]`)',
    evidence: { file: 'tests/.constructs/matcher-discrimination.tflw', pattern: '^\\s*expect body is empty$', min: 1 },
    graders: ['acceptance'],
    knownAnswer: 'a non-empty list and a non-empty string are `not empty`, and an empty search result is `empty`; the mutation control turns both negatives red.',
    catches: 'an emptiness test that answers one way for every subject, or that reads a string or a list as its truthiness.',
    blockedOn: null,
  },
  // tflw `M242` `B`/`D` (`D1327`, `D1329`) — one plant file, which also carries the GraphQL and
  // `capture … matching` journeys (graded under `C124`, because neither is a manifest construct).
  {
    id: 'C124',
    construct: 'generator:transform-length',
    family: 'generator',
    tier: 'api',
    title: '`length of` reads what `.length` reads',
    target: 'tests/.constructs/language-m242.tflw against the known-answer payload',
    evidence: { file: 'tests/.constructs/language-m242.tflw', pattern: '= length of ', min: 2 },
    graders: ['acceptance'],
    knownAnswer: 'two for the two-element `tags` (and not one), twelve for `"known-answer"`; the same file\'s GraphQL query and `capture … matching` journeys must pass too.',
    catches: 'a length that counts something else (characters of the JSON, the last index), and a regression in the plant\'s two journeys.',
    blockedOn: null,
  },
  {
    id: 'C125',
    construct: 'generator:transform-join',
    family: 'generator',
    tier: 'api',
    title: '`joined with` puts the separator between the elements, in order',
    target: 'the same plant as `C124`',
    evidence: { file: 'tests/.constructs/language-m242.tflw', pattern: ' joined with ', min: 1 },
    graders: ['acceptance'],
    knownAnswer: '`"plant, soft-assertion"`, and not the reverse order.',
    catches: 'a join that reorders, drops the separator, or stringifies the list whole.',
    blockedOn: null,
  },
  {
    id: 'C126',
    construct: 'declaration:skip',
    family: 'declaration',
    tier: 'api',
    title: 'a skipped test runs nothing, says why, and does not fail the run',
    target: 'the last test of the `C124` plant, whose body would fail (`expect status equals 599`)',
    evidence: { file: 'tests/.constructs/language-m242.tflw', pattern: '^test ".*" skip "planted by C126', min: 1 },
    graders: ['acceptance'],
