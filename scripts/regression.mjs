...
const PHASES = [
  ...
  // `S-3d` infrastructure (decision 18): the api over IPv6 loopback, by a bracketed base URL.
  {
    name: 'ipv6-check',
    cmd: [TFLW, 'run', '--no-color', ...CI_VERBOSE, '--env', 'ipv6Loopback', 'tests/.env-specific/ipv6-loopback.tflw'].join(' '),
  },
  {
    name: 'mtls-rejection',
  ...
  // `S-3d` infrastructure (decision 18): tflw through the tinyproxy tenant, SPEC §3.5's route.
  { name: 'proxy-check', cmd: 'node scripts/verify-proxy.mjs' },
