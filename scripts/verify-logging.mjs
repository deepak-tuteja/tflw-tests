...
const TFLW = tflwCommand('released', { label: 'verify-logging' });
...
{
  const { stdout } = run(`${TFLW} run --env logConfig tests/api/mechanics/logging.tflw --no-color`);
...
{
  const { stdout } = run(`${TFLW} run --env logConfig tests/api/mechanics/logging.tflw --no-color --log-level debug`);
...
{
  const { stdout } = run(`${TFLW} run --env logConfig tests/api/mechanics/logging.tflw --no-color --log-output html`);
  ok(
...
{
  const { stdout } = run(`${TFLW} run --env local tests/api/mechanics/logging.tflw --no-color --log-output console`);
