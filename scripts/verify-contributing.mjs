...
const CLASSIFIED = [
  ...
  },
  {
    wf: 'ci.yml',
    job: 'acceptance-check',
    cmd: 'npm run verify:grader-reachability',
    name: 'Every grader in scripts/ is reachable from something that runs',
    class: 'gate',
    local: 'npm run verify:grader-reachability',
    why: '`M163-01`/`D828` — the audit that found `verify-screenshot-step.mjs` asserting four known answers with zero callers was a hand pass, and three earlier instances of the same shape were each found by somebody who happened to be editing that file for another reason. A contributor gate rather than ci-only because the failure it catches is *created* locally, in the same edit that adds a script: the cheapest moment to be told is before the push, and it costs one node start with no stack and no sibling checkout',
  },
  {
    wf: 'ci.yml',
    job: 'acceptance-check',
    cmd: 'npm run verify:grader-reachability:self-test',
    name: "The reachability gate's own controls fire on the input they exist for",
    class: 'gate',
    local: 'npm run verify:grader-reachability:self-test',
    why: '`M172e`\'s rule for a gate that is green the day it lands: the unreachable set and the declared `EXEMPT` set are the same three files, so the real run cannot go red today and a permanent green is later read as an argument for deletion. These seven controls are the proof it can refuse — including the two directions of the hand list (`D895`), an entry naming a file that does not exist and an exemption that has become stale',
  },
  {
    wf: 'ci.yml',
    job: 'acceptance-check',
  ...
  },
  {
    wf: 'ci.yml',
    job: 'acceptance-check',
    cmd: 'npm run verify:reach:self-test',
    name: "The reach fold's controls fire",
    class: 'gate',
    local: 'npm run verify:reach:self-test',
    why: "`scripts/lib/reach.mjs` — `M189a` (`D974`): the source-map decode, the innermost-range sweep over V8 block coverage and the `find`-region location behind `measure-mutation-reach.mjs`. Its controls are the VLQ known vectors, a fold where an outer count-1 must not rescue an inner count-0 and a second process ORs in, a region whose guard line ran and whose body did not, an unmapped region that must read `unmapped` and not `unreached`, and the alignment control that catches offsets which are not the file's own. A contributor gate because it is milliseconds with no stack, and because the thing it catches — a fold that has quietly stopped matching V8's or esbuild's output — would otherwise surface as a plausible reach written two box-hours later",
  },
  {
    wf: 'ci.yml',
    job: 'acceptance-check',
    cmd: 'npm run verify:reach-verdicts:self-test',
    name: "The reach reading's controls fire",
    class: 'gate',
    local: 'npm run verify:reach-verdicts:self-test',
    why: "`scripts/lib/reach-verdicts.mjs` — the bins over the census's surviving runtime mutants and every way the hand table can disagree with them: a reached survivor with no verdict, a verdict about an unreached, killed, non-runtime or unlocated mutation, a verdict outside the two-word vocabulary, one with no family or a one-word why, and a missing, stampless or partial measurement, each of which must refuse. Milliseconds, no stack",
  },
  {
    wf: 'ci.yml',
    job: 'acceptance-check',
    cmd: 'npm run verify:reach-verdicts',
    name: 'Every reached survivor has a verdict, and every verdict names a reached survivor',
    class: 'gate',
    local: 'npm run verify:reach-verdicts',
    why: "`M189a` (`D974`). `tflw-acceptance/mutation/reach.json` is measured — one instrumented roster run per census, on the box — and `reach-verdicts.json` is hand-read, one verdict per reached survivor: `not-asserted` (a plant runs through it and its known answer does not depend on the result) or `out-of-reach-by-design` (visible only where the roster does not look). This holds the two to each other in both directions, `mutation-covers.mjs`'s arrangement (`D842`, `D767`), and refuses outright when the measurement is absent or partial, because a gate green about nothing is `D722`. A contributor gate because it reads three committed files and runs nothing",
  },
  {
    wf: 'ci.yml',
    job: 'acceptance-check',
    cmd: 'npm run verify:contention:self-test',
    name: "The contention gate fires on the incident's shape",
    class: 'gate',
    local: 'npm run verify:contention:self-test',
    why: "`scripts/lib/box-contention.mjs` — `M190b` (`D991`). The census asks the box before its baseline and before every window close, through `statsctl tenants` and `statsctl check --for tflw:load`, and refuses beside a render in flight, a running model server or a memory stall. The decision is a pure function over the two payloads, and its controls are the 2026-09-13 22:40 incident's own shape (a render queued, psi high) against the 2026-09-14 10:40 one (ComfyUI resident and idle, a clean sweep beside it) — resident is not the hazard, rendering is. Milliseconds, no box, no render",
  },
  {
    wf: 'ci.yml',
    job: 'acceptance-check',
    cmd: 'npm run verify:exec-argv:self-test',
    name: 'The driver refuses a detaching command',
    class: 'gate',
    local: 'npm run verify:exec-argv:self-test',
    why: "`scripts/lib/exec-argv.mjs` — `M190b` (`D992`, `M190-03`). `exec.mjs exec -- '… &'` never returned: the detached child kept the remote wrapper alive and the driver's `tflw:exec` lease with it. The driver now refuses a command whose last token is `&` and names the box-side rule; the controls are the overnight launch verbatim, `&&` and `2>&1` as the non-detaching neighbours, and the refusal text. Milliseconds",
  },
  {
    wf: 'ci.yml',
    job: 'acceptance-check',
